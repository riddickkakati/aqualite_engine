...
class Air2water_OOP:
    ...
    def __init__(self,uuid=None,spot_setup=spot_setup, interpolate=True, n_data_interpolate=7, validation_required=True,
                 model = "air2water", core=1, depth = 14.0, swarmsize=20, phi1=2.0, phi2=2.0, omega=0.5, maxiter=20, numbersim= 2000, method = 'SpotPY', mode = "calibration", error="RMSE", kge_button=True, rmse_button=True, ns_button=True, db_file = "SCEUA_hymod.db",
    ...
    def run(self):
        run_directory = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
        if self.uuid==None:
            ...
            count_file = str(run_directory[:-4] + os.sep +"run_count.txt")
            ...
            try:
                ...
                # If the file doesn't exist, start the count from 0
                run_count = 0
            # Increment the count each time the code is run
            run_count += 1
            ...
            datafolder = str(run_directory[:-4] + os.sep + str('runs') + os.sep + str(f"{run_count}"))
            ...
            if self.mode != "forward":
                ...
                if self.computeparametersranges == "No":
                    ...
                    shutil.copy(self.parameter_ranges,
                                datafolder + os.sep + str(f'{run_count}_{self.model}_{self.mode}_parameters.yml'))
                else:
            ...
            all_parameters1 = read_yaml(datafolder + os.sep + str(f'{run_count}_{self.model}_{self.mode}_parameters.yml'))
            ...
            shutil.copy(usercalibrationdatapath, datafolder + os.sep + str(f'{run_count}_calibration.txt'))

            uservalidationdatapath = self.uservalidationpath
            if self.validation_required == False:
                df2 = np.loadtxt(uservalidationdatapath)
                shutil.copy(uservalidationdatapath, datafolder + os.sep + str(f'{run_count}_validation.txt'))

            processor = YearlyDataProcessor(df1,self.n_data_interpolate)
            dfint1, num_missing_col3, missing_col3 = processor.mean_year()
            np.savetxt(datafolder + os.sep + str(f'{run_count}_{self.model}_{self.mode}_calibration.csv'), dfint1, delimiter=",", fmt='%s')
            ...
            if self.interpolate == True and num_missing_col3:
                ...
                if self.validation_required == False:
                    ...
                    np.savetxt(datafolder + os.sep + str(f'{run_count}_{self.model}_{self.mode}_validation.csv'), dfint2, delimiter=",", fmt='%s')
                    calibration = dfint1
                    validation = dfint2
                else:
            ...
            if self.mode != "forward":
                ...
                db_file_location = str(owd + os.sep + self.spot_setup.db_file)
                shutil.copy(db_file_location, datafolder + os.sep + str(f'{run_count}_{self.optimizer}_results.db'))
            ...
            shutil.copy(results_file_location, datafolder + os.sep + str(f'{run_count}_results.db'))
            ...
            if self.email_send == 1:
                send_email(self.model, self.email_list, datafolder + os.sep + str(f'{run_count}_results.db'))
        else:
            keyfound=None
            yaml_file_path=run_directory + os.sep +  "air2water" + os.sep + "UUIDs.yaml"
            with open(yaml_file_path, "r") as yaml_file:
                uuid_dict = yaml.safe_load(yaml_file)

            for key, value in uuid_dict.items():
                if value == self.uuid:
                    keyfound=key
            if keyfound==None:
                print("Simulation does not exist. Please rerun.")
                exit()
            else:
                datafolder = str(run_directory[:-4] + os.sep + str('runs') + os.sep + str(f"{keyfound}"))
            print(datafolder)
        ...
        return run_count,num_missing_col3
