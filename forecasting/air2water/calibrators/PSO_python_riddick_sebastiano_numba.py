...
@jit(nopython=True)
def update_particles(x, v, pbest, fit, fitbest, gbest, par_min, par_max, w, c1, c2, n_parameters, n_particles):
    """
    Numba-optimized particle update function
    """
    best_changed = False
    foptim = np.max(fitbest)

    for k in range(n_particles):
        r = np.random.uniform(0, 1, 2 * n_parameters)
        status = 0

        # Update velocity and position
        for j in range(n_parameters):
            v[j, k] = (w * v[j, k] +
                       c1 * r[j] * (pbest[j, k] - x[j, k]) +
                       c2 * r[j + n_parameters] * (gbest[j] - x[j, k]))
            x[j, k] = x[j, k] + v[j, k]

            # Absorbing wall boundary conditions
            if x[j, k] > par_max[j]:
                if j == 5:  # Special case for parameter 6
        ...
        # Mark particle for fitness recalculation
        if status == 0:
            fit[k] = -2e30  # Special value to indicate need for recalculation
        else:
            fit[k] = -1e30
...
class pso_oop:
    def __init__(self, objective_function, db_file, db_format, par_min: List[float], par_max: List[float],
                 ...
                 min_eff_index: float = 0.0, minstep: float = 0.001):
        """
        ...
        self.n_runs = maxiter
        self.par_min = np.array(par_min, dtype=np.float64)
        self.par_max = np.array(par_max, dtype=np.float64)
        self.w_max = w_max
        self.w_min = w_min
        self.c1 = phip
        self.c2 = phig
    ...
    def run(self) -> Tuple[np.ndarray, float]:
        ...
        # Initialize particles with proper dtype for Numba
        x = np.random.uniform(0, 1, (self.n_parameters, self.n_particles)).astype(np.float64)
        v = np.random.uniform(0, 1, (self.n_parameters, self.n_particles)).astype(np.float64)
        pbest = np.zeros_like(x, dtype=np.float64)
        fit = np.zeros(self.n_particles, dtype=np.float64)
        fitbest = np.zeros(self.n_particles, dtype=np.float64)
        ...
        for i in range(self.n_runs):
            ...
            x, v, fit = update_particles(x, v, pbest, fit, fitbest, gbest,
                                         self.par_min, self.par_max, w, self.c1, self.c2,
                                         self.n_parameters, self.n_particles)

            # Recalculate fitness for particles that need it
            for k in range(self.n_particles):
                if fit[k] == -2e30:  # Special value indicating need for recalculation
                    parameters = x[:, k].copy()
                    fit[k] = self.objective_function(parameters)
            ...
            # Progress reporting and early stopping remain the same...
            if i >= 10 and i % (self.n_runs // 10) == 0:
                print(
                    f'{100.0 * i / self.n_runs:.1f}% done. Minimum objective function: {abs(foptim)}. Parameters: {abs(gbest)}')
