...
class PSO:
    def __init__(self, n_particles: int, n_parameters: int, param_bounds: np.ndarray,
                 objective_func: Callable, n_iterations: int = 2000,
                 w_max: float = 0.9, w_min: float = 0.4, c1: float = 2.0, c2: float = 2.0,
                 min_eff_index: float = -np.inf, norm_min: float = 0.001):
        """
        ...
        Args:
            ...
            param_bounds: Array of shape (n_parameters, 2) containing min and max bounds
            ...
            norm_min: Minimum norm for convergence check
        """
        ...
        # Initialize arrays
        self.x = np.zeros((n_parameters, n_particles))
        self.v = np.zeros((n_parameters, n_particles))
        self.pbest = np.zeros((n_parameters, n_particles))
        self.gbest = np.zeros(n_parameters)
        self.fit = np.zeros(n_particles)
        self.fitbest = np.zeros(n_particles)

    def initialize_particles(self):
        """Initialize particle positions and velocities randomly within bounds"""
        for j in range(self.n_parameters):
            dxmax = self.param_bounds[j, 1] - self.param_bounds[j, 0]
            dvmax = 1.0 * dxmax

            # Random initialization within bounds
            self.x[j, :] = np.random.random(self.n_particles) * dxmax + self.param_bounds[j, 0]
            self.v[j, :] = np.random.random(self.n_particles) * dvmax
            self.pbest[j, :] = self.x[j, :]

        # Evaluate initial fitness
        for k in range(self.n_particles):
            self.fit[k] = self.objective_func(self.x[:, k])
            self.fitbest[k] = self.fit[k]

        # Set initial global best
        best_idx = np.argmax(self.fit)
        self.gbest = self.x[:, best_idx]

    def optimize(self):
        """Run the PSO optimization"""
        print(f"N. particles = {self.n_particles}, N. iterations = {self.n_iterations}")

        self.initialize_particles()
        w = self.w_max
        dw = (self.w_max - self.w_min) / self.n_iterations

        # Main optimization loop
        for i in range(self.n_iterations):
            for k in range(self.n_particles):
                r1 = np.random.random(self.n_parameters)
                r2 = np.random.random(self.n_parameters)
                status = 0

                # Update velocity and position
                self.v[:, k] = (w * self.v[:, k] +
                                self.c1 * r1 * (self.pbest[:, k] - self.x[:, k]) +
                                self.c2 * r2 * (self.gbest - self.x[:, k]))
                self.x[:, k] = self.x[:, k] + self.v[:, k]

                # Absorbing wall boundary conditions
                for j in range(self.n_parameters):
                    if self.x[j, k] > self.param_bounds[j, 1]:
                        if j == 5:  # Special case for parameter 6 (index 5)
                            self.x[j, k] = self.x[j, k] - np.floor(self.x[j, k])
                        else:
                            self.x[j, k] = self.param_bounds[j, 1]
                            self.v[j, k] = 0.0
                            status = 1

                    if self.x[j, k] < self.param_bounds[j, 0]:
                        if j == 5:  # Special case for parameter 6 (index 5)
                            self.x[j, k] = np.ceil(abs(self.x[j, k])) - abs(self.x[j, k])
                        else:
                            self.x[j, k] = self.param_bounds[j, 0]
                            self.v[j, k] = 0.0
                            status = 1

                # Evaluate fitness if particle is valid
                if status == 0:
                    self.fit[k] = self.objective_func(self.x[:, k])

                    # Update personal best
                    if self.fit[k] > self.fitbest[k]:
                        self.fitbest[k] = self.fit[k]
            ...
            # Update global best
            best_idx = np.argmax(self.fitbest)
            ...
            # Progress reporting
            if i >= 10 and i % (self.n_iterations // 10) == 0:
                print(f"{100.0 * i / self.n_iterations:.1f}% done")

            # Convergence check
            count = 0
            for k in range(self.n_particles):
                norm = np.sqrt(np.sum(((self.pbest[:, k] - self.gbest) /
                                       (self.param_bounds[:, 1] - self.param_bounds[:, 0])) ** 2))
                if norm < self.norm_min:
                    count += 1

            if count >= 0.90 * self.n_particles:
                print("- Warning: PSO has converged early")
                break

        final_fit = self.objective_func(self.gbest)
        print(f"Calibration: objective function = {abs(final_fit)}")

        return self.gbest, final_fit
...
if __name__ == "__main__":
    ...
    n_particles = 2000
    n_parameters = 8
    parameters1 = np.array([s.lb, s.ub]).T
    param_bounds = parameters1  # Same bounds for all parameters
    # Create and run PSO
    for n in range(30):
        ...
        output_file_path = owd + os.sep + str(
            f'04.10_PSO_own_a2w_all.txt')  # Change fortran/cython in core.py
