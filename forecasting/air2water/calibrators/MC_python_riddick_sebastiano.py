...
def monte_carlo_sampling(n_runs, n_params, param_min, param_max, objective_func,
                         min_eff_index=-999):
    """
    Monte Carlo sampling algorithm

    Args:
        n_runs: Number of random samples
        n_params: Number of parameters
        param_min: List of minimum parameter values
        param_max: List of maximum parameter values
        objective_func: Function to evaluate parameters
        log_params: List of parameter indices to apply log transformation
        min_eff_index: Minimum efficiency index threshold
    """
    print(f'N. runs = {n_runs}')

    # Initialize best solution tracking
    gbest = np.zeros(n_params)
    foptim = -999

    # Main sampling loop
    for i in range(n_runs):
        # Generate random parameters
        r = np.random.random(n_params)
        parameters = param_min + (param_max - param_min) * r

        # Apply log transformation if specified

        # Evaluate parameters
        eff_index = objective_func(parameters)

        # Update best solution if improvement found
        if eff_index > foptim:
            foptim = eff_index
            gbest = parameters.copy()

        # Progress reporting
        if i >= 10 and i % (n_runs // 10) == 0:
...
if __name__ == "__main__":
    ...
    best_params, best_fitness = monte_carlo_sampling(
        n_runs=n_runs,
        n_params=n_params,
        param_min=param_min,
        param_max=param_max,
